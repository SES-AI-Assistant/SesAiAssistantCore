...
### Java/Maven 開発ルール
- **Java Version:** pom.xmlを参照
- **Build Command:** `mvn clean compile` を基本とする。
- **Eclipse 連携:**
    - `mvn` コマンド成功後は、必ず `mvn eclipse:eclipse` や `.classpath` の更新を意識した出力をすること。
...
### 6.1 作業開始時の「儀式」（documents/INSTRUCTION.md と TODO.md の活用）
1. **指示の確認と抽出**: 開発を開始する際は、まず `documents/INSTRUCTION.md` を読み込み、**最も新しく追加された未完了のタスク**を特定してください。
...
4. **完了後のクリーンアップ**: タスクが完了（検証まで成功）したら、実行した指示内容を `documents/ARCHIVE_INSTRUCTIONS.md` に追記し、`documents/INSTRUCTION.md` を次の指示が書けるよう初期状態（テンプレート）に戻してください。
5. **コンテキスト同期**: 開発を再開する際は必ず `read_file("TODO.md")` を行い、前回の状態を完全に把握してから行動してください。
6. **設計書最新化**: 開発が完了したら必ずdocuments/配下の設計書のうち、更新が必要なもののみを更新または追記してください。特にフローチャート図がある場合は正しく反映してください。
...
- **自動テスト**: 修正後は必ず関連するテストコードを実行（`mvn test` 等）し、成功を確認してください。
    - カバレッジは100%を常に担保してください。
    - カバレッジレポートをファイルに出力してからタスクを完了してください
    - テストクラスとテスト対象のクラスは1対1で対応する事を必須とし、テストケースを複数のファイルに分離させることは禁止です
    - mvn clean install を実行してビルドが通る事を確認してください
    - mvn test を実行してテストが通りカバレッジを確認してください
- **リファクタリング**: テスト通過後、命名規則やコードの綺麗さがプロジェクトの掟に沿っているかセルフチェックを行ってください。
- **静的コードチェック**:
※下記のmvnコマンドが失敗する場合、適宜pom.xmlにライブラリを追加し実行できるようにすること。
    - mvn rewrite:run を実行して、プロジェクト全体の未使用インポートを削除し、コードスタイルを修正すること。
    - mvn spotless:apply を実行してコードを綺麗にすること。
    - mvn checkstyle:check を実行してチェックスタイルを確認し違反している場合は修正する事
    - mvn pmd:pmd を確認し違反している場合は修正する事
    - mvn pmd:cpd を確認し違反している場合は修正する事
    - mvn spotbugs:check を確認し違反している場合は修正する事
