...
public class Content {
    ...
    public Content(final String rawContent) {
        ...
        this.contentList = new ArrayList<String>();
        this.targetNumberOfCriteria = parseTargetCriteria();
        this.jobScore = calcScore(rawContent, PROP_JOB_FEATURES_HIGH, PROP_JOB_FEATURES_LOW, PROP_JOB_FEATURES);
        this.personelScore = calcScore(rawContent, PROP_PERSON_FEATURES_HIGH, PROP_PERSON_FEATURES_LOW,
                PROP_PERSON_FEATURES);
    }
    ...
    /**
     * テキストに対してワードの重み付きスコアを計算する.
     * 高スコアワード設定が存在する場合はそちらを優先し、
     * 存在しない場合は後方互換として全ワードを低スコアで計算する。
     *
     * @param text          対象テキスト
     ...
     * 判定基準文字数以上かつ案件スコアが要員スコアよりも高い場合に true を返します。
     *
     * @return 案件紹介文と判定すればtrue、それ以外はfalse
     */
    public boolean is案件紹介文() {
        return !this.isEmpty()
                ? (this.rawContent.length() >= this.targetNumberOfCriteria) && (this.jobScore > this.personelScore)
                : false;
    }

    /**
     * このメッセージが要員の紹介文であるかどうかを判定します.
     * 判定基準文字数以上かつ要員スコアが案件スコアよりも高い場合に true を返します。
     *
     * @return 要員紹介文と判定すればtrue、それ以外はfalse
     */
    public boolean is要員紹介文() {
        return !this.isEmpty()
                ? (this.rawContent.length() >= this.targetNumberOfCriteria) && (this.personelScore > this.jobScore)
                : false;
    }
