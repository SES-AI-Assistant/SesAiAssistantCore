...
public @interface Schema {
  ...
  /**
   * 年の指定を今年および翌年のみに限定するかどうか（文字列型用）.
   *
   * <p>trueの場合、スキーマ生成時に実行時のシステム年（今年）と翌年のみを許容する正規表現パターンが動的に設定されます。
   * patternが指定されている場合は年部分（\\d{4}または[0-9]{4}）が置換され、未指定の場合はデフォルトの年月パターン（例: ^(2026|2027)/(0?[1-9]|1[0-2])$）が設定されます。
   *
   * @return 今年および翌年のみに限定する場合true。デフォルトはfalse
   */
  boolean currentAndNextYearOnly() default false;
}
