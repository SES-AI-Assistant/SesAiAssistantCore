...
public class MatchEvaluateSummary {
  ...
  public boolean isMatch() {
      ...
      boolean isAllMustMet = this.mustList.stream()
              .allMatch(must -> must.getResult() == EvaluateType.FullyMet);
      if (!isAllMustMet) {
          return false;
      }
      // 3. 案件単価要件
  ...
  /**
   * マッチング詳細テーブルの評価文カラムにセットする文字列に変換する.
   * 最大2000文字.
   *
   * @return 評価文
   */
  public String toEvaluiationText() {
    ...
    if (this.mustList != null && !this.mustList.isEmpty()) {
        ...
        for (SkillEvaluateResult must : this.mustList) {
            sb.append("・").append(must.getPerspective()).append("：")
    ...
    if (this.wantList != null && !this.wantList.isEmpty()) {
        ...
        for (SkillEvaluateResult want : this.wantList) {
            sb.append("・").append(want.getPerspective()).append("：")
    ...
    if (this.officeResult != null) {
        String icon = this.officeResult.getResult() != null ? this.officeResult.getResult().getIcon() : "-";
    ...
    if (this.placeResult != null) {
        String icon = this.placeResult.getResult() != null ? this.placeResult.getResult().getIcon() : "-";
        sb.append("■場所：");
    ...
    if (this.priceResult != null && !OriginalStringUtils.isEmpty(this.priceResult.getComment())) {
        ...
        sb.append("■単価：").append(icon)
          .append("（").append(this.priceResult.getComment()).append("）\n");
    }
    // 7. 人月工数（必要に応じて出力）
    if (this.personMonthsResult != null && !OriginalStringUtils.isEmpty(this.personMonthsResult.getComment())) {
        String icon = this.personMonthsResult.isResult() ? "○" : "×";
        sb.append("■人月工数：").append(icon)
          .append("（").append(this.personMonthsResult.getComment()).append("）\n");
    }
    // 8. その他
