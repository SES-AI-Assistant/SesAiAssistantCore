...
public class LineMessagingAPI {
    ...
    try {
      ...
      for (final String message : this.messageList) {
        ...
        if (response.statusCode() != HttpURLConnection.HTTP_OK) {
          log.warn(
              "【SesAiAssitantCore】LINE Messaging APIでのメッセージ送信中にエラーが発生しました：{}",
              response.statusCode());
        }
      }
    } catch (Exception e) {
  ...
  public void broadCast() {
    try {
      ...
      if (response.statusCode() != HttpURLConnection.HTTP_OK) {
        log.warn(
            "【SesAiAssitantCore】LINE Messaging APIでのメッセージ配信中にエラーが発生しました：{}", response.statusCode());
      }
    } catch (Exception e) {
      e.printStackTrace();
    }
  ...
  public byte[] getFile(final String messageId) throws IOException, InterruptedException {
    ...
    if (response.statusCode() == HttpURLConnection.HTTP_OK) {
      ...
      log.warn(
          "【SesAiAssitantCore】LINE Messaging APIでのファイルダウンロード中にエラーが発生しました：{}",
          response.statusCode());
      return null;
    }
  }
