...
public class SES_AI_API_USAGE_HISTORY extends DynamoDB<SES_AI_API_USAGE_HISTORY> {
     ...
     * 【PK】
     * モデル名.
     * 例：GPT4o-mini、Gemini-1.5-flashなど
     */
     ...
     * 【SK】
     * APIを使用したユーザーのID.
     * システムがAPIを利用した場合はモジュール名が入力されます
     * 例：ユーザーID、lambda関数名など
     */
     ...
     * 【SK】
     *  API種別.
     *  例：Generate、Embedding、FineTuningなど
     */
    ...
    @Override
    public void save() {
        if (this.provider == null || this.model == null || this.model.isEmpty() || this.usageMonth == null || this.usageMonth.isEmpty()) {
            return;
        } else {
    ...
    @Override
    public void delete() {
        this.table.deleteItem(Key.builder().partitionValue(this.getPartitionKey()).sortValue(this.getSortKey()).build());
    }

    @Override
    public void fetch() {
        SES_AI_API_USAGE_HISTORY latest = this.table.getItem(r -> r
                .key(Key.builder().partitionValue(this.getPartitionKey()).sortValue(this.getSortKey()).build())
                .consistentRead(true)
        );
        if (latest != null) {
