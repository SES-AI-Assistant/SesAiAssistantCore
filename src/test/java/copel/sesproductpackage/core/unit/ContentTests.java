...
class ContentTests {
  ...
  /** 要員弱ワード（LOW: 1点） */
  private static final String PERSON_LOW =
      "自己,性格,周囲,勤怠,職歴,アピール,人物,対人,正社員,取得,試験,人柄,キャッチアップ,忍耐,体力,主体性,作業,希望,活躍";

  /** 後方互換用 案件ワード */
  private static final String JOB_FEATURES =
      "エンド,エンド直,支払サイト,支払いサイト,募集,提案,外国,期間,就業,歓迎,支払い,制限,契約,精算,向け,見合い,優遇,有識者,プロジェクト,若手,概要,場所,内容,時期,前後,作業";

  /** 後方互換用 要員ワード */
  private static final String PERSON_FEATURES =
      "氏名,名前,本人,自己,性格,周囲,勤怠,職歴,アピール,人物,対人,稼働,資格,正社員,応相談,取得,性別,試験,人柄,男性,女性,キャッチアップ,忍耐,体力,主体性,作業,最寄り駅,希望,性別,直近,活躍";
  ...
  void testIs案件紹介文() {
    ...
    String text =
        "案件のご紹介です。エンド直の案件となります。精算幅は140H〜180Hです。"
  ...
  void testIs案件紹介文_実データ5種() throws Exception {
    ...
    for (String file : jobFiles) {
      ...
      assertTrue(
          content.is案件紹介文(),
          "案件として判定されるべきファイルが正しく判定されませんでした: " + file + " [文字数=" + text.length() + "]");
      assertFalse(content.is要員紹介文(), "案件ファイルが要員紹介文と誤判定されました: " + file);
    }
  }
  ...
  void testIs要員紹介文_実データ5種() throws Exception {
    ...
    for (String file : personFiles) {
      ...
      assertTrue(
          content.is要員紹介文(),
          "要員として判定されるべきファイルが正しく判定されませんでした: " + file + " [文字数=" + text.length() + "]");
      assertFalse(content.is案件紹介文(), "要員ファイルが案件紹介文と誤判定されました: " + file);
    }
  }
