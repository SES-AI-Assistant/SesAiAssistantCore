...
class AAA_PropertiesTest {
  ...
  void testStaticBlockException() {
    // This test attempts to cover the catch block in the static initializer of the Properties class.
    ...
    try (MockedStatic<S3Client> s3Mock = mockStatic(S3Client.class)) {
      // Force the builder() method to throw an exception, which should be caught by the static block
